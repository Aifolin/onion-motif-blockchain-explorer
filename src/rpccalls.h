...
#ifndef CROWMTF_RPCCALLS_H
#define CROWMTF_RPCCALLS_H
...
{
// declare struct in motif's cryptonote namespace.
// motif should provide definition for this,
...
public:
    ...
    get_alt_blocks(vector<string>& alt_blocks_hashes)
    {
        ...
        if (r)
        {
            ...
            if (!err.empty())
            {
                cerr << "Error connecting to Motif deamon due to "
                     << err << endl;
                return false;
            }
        }
        ...
        {
            cerr << "Error connecting to Motif deamon at "
                 << deamon_url << endl;
            return false;
        }
