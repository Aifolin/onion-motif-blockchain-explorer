...
#ifndef MTFEG01_TOOLS_H
#define MTFEG01_TOOLS_H

#define PATH_SEPARARTOR '/'

#define MTF_AMOUNT(value) \
...
static
string
mtf_amount_to_str(const uint64_t& mtf_amount,
                  ...
                  bool zero_to_question_mark=true)
{
    ...
    {
        amount_str = fmt::format(_format, MTF_AMOUNT(mtf_amount));
    }
    else
    {
        if (mtf_amount > 0 && zero_to_question_mark == true)
        {
            amount_str = fmt::format(_format, MTF_AMOUNT(mtf_amount));
        }
    }
