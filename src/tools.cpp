...
/**
* Parse motif address in a string form into
...
/**
* Return string representation of motif address
*/
string
...
{
    // default path to motif folder
    // on linux this is /home/<username>/.bitmotif
    string default_motif_dir = tools::get_default_data_dir();

    if (nettype == cryptonote::network_type::TESTNET)
        default_motif_dir += "/testnet";
    if (nettype == cryptonote::network_type::STAGENET)
        default_motif_dir += "/stagenet";


    // the default folder of the lmdb blockchain database
    // is therefore as follows
    return default_motif_dir + string("/lmdb");
}
...
{
    // the default folder of the lmdb blockchain database
    string default_lmdb_dir   = mtfeg::get_default_lmdb_folder(nettype);
    ...
    blockchain_path = mtfeg::remove_trailing_path_separator(blockchain_path);
...
{
    pair<uint64_t, uint64_t> sum_mtf {0, 0};
    ...
    {
        sum_mtf.first += vout["amount"].get<uint64_t>();
        ++sum_mtf.second;
    }
...
{
    pair<uint64_t, uint64_t> sum_mtf {0ULL, 0ULL};
        ...
        vector<txin_to_key>& input_key_imgs)
{
    ...
    uint64_t mtf_outputs       {0};
    uint64_t mtf_inputs        {0};
    uint64_t mixin_no          {0};
    ...
    for (size_t i = 0; i < input_no; ++i)
    {
        ...
        mtf_inputs += tx_in_to_key.amount;
    ...
    return {mtf_outputs, mtf_inputs, mixin_no, num_nonrct_inputs};
};
...
summary_of_in_out_rct(const json& _json)
{
    ...
    for (const json& vout: _json["vout"])
    {
        mtf_outputs += vout["amount"].get<uint64_t>();
    }
    ...
    return {mtf_outputs, mtf_inputs, no_outputs, no_inputs, mixin_no, num_nonrct_inputs};
};
...
sum_money_in_inputs(const string& json_str)
{
    ...
    for (json& vin: j["vin"])
    {
        sum_mtf.first += vin["key"]["amount"].get<uint64_t>();
        ++sum_mtf.second;
    }
...
{
    array<uint64_t, 2> sum_mtf;

    sum_mtf[0] = sum_money_in_inputs(tx);
    sum_mtf[1] = sum_money_in_outputs(tx);
