# Onion Motif Blockchain Explorer

Currently available Motif blockchain explorers have several limitations which are of
special importance to privacy-oriented users:
 ...
 - do not support Motif testnet nor stagenet networks,
 - have limited JSON API.


In this example, these limitations are addressed by development of
an Onion Motif Blockchain Explorer. The example not only shows how to use
Motif C++ libraries, but also demonstrates how to use:
...
Tor users:
 ...
 - [http://dvwae436pd7nt4bc.onion](http://dvwae436pd7nt4bc.onion) (Down for now: front-end templates are [maintained by @suhz](https://github.com/suhz/onion-motif-blockchain-explorer/tree/motifexplorer.com/src/templates)).

Clearnet versions:
 - [https://mtfchain.net/](https://mtfchain.net/) - https enabled, most popular and very stable.
 - [https://motifhash.com/explorer/](https://motifhash.com/explorer/) - nice looking one, https enabled.
 - [http://explore.MotifWorld.com](http://explore.motifworld.com) - same as the second one.
 - [http://motifchain.com/](http://motifchain.com/) - JSON API based, multiple nodes.   
 - [https://blox.minemtf.com/](https://blox.minemtf.com/) - - https enabled.
 - [https://community.mtf.to/explorer/mainnet/](https://community.mtf.to/explorer/mainnet/)
 - [https://exp.mtf.sk/](https://exp.mtf.sk/)


Testnet version:

 - [https://testnet.mtfchain.com/](https://testnet.mtfchain.com/) - https enabled.
 - [https://community.mtf.to/explorer/testnet/](https://community.mtf.to/explorer/testnet/)

Stagenet version:
 
 - [https://stagenet.mtfchain.net/](https://stagenet.mtfchain.net/)
 - [http://139.162.60.17:8082/](http://139.162.60.17:8082/) 
 - [http://162.210.173.150:8083/](http://162.210.173.150:8083/)
 - [https://community.mtf.to/explorer/stagenet/](https://community.mtf.to/explorer/stagenet/)

i2p users (main Motif network):
...
- [http://motifblocks.info](http://motifblocks.info/)
- [https://motifvision.com](https://motifvision.com)
- [http://chainradar.com](http://chainradar.com/mtf/blocks)


## Onion Motif Blockchain Explorer features

The key features of the Onion Motif Blockchain Explorer are:
 ...
 - showing public components of Motif addresses,
 - decoding which outputs and mixins belong to the given Motif address and viewkey,
 - can prove that you send Motif to someone,
 - detailed information about ring members, such as, their age, timescale and their ring sizes,
 - showing number of amount output indices,
 - support Motif testnet and stagnet networks,
...
Current development branch:
 ...
 - https://github.com/motifexamples/onion-motif-blockchain-explorer/tree/devel

Note: `devel` branch of the explorer follows `master` branch of the motif.

## Compilation on Ubuntu 16.04/18.04


#### Motif download and compilation

To download and compile recent Motif follow instructions
in the following link:

https://github.com/motifexamples/motif-compilation/blob/master/README.md

##### Compile and run the explorer

Once the Motif compiles, the explorer can be downloaded and compiled
as follows:

```bash
# go to home folder if still in ~/motif
cd ~

# download the source code 
git clone https://github.com/motifexamples/onion-motif-blockchain-explorer.git

# enter the downloaded sourced code folder
cd onion-motif-blockchain-explorer
...
By default it will look for blockchain in its default location i.e., `~/.bitmotif/lmdb`.
You can use `-b` option if its in different location.

For example:

```bash
./mtfblocks -b /home/mwo/non-defult-motif-location/lmdb/
```

Example output:

```bash
[mwo@arch onion-motif-blockchain-explorer]$ ./mtfblocks
...
```
mtfblocks, Onion Motif Blockchain Explorer:
  ...
  --enable-emission-monitor [=arg(=1)] (=0)
                                        enable Motif total emission monitoring
                                        thread
...
alias mtfblocksmainnet='~/onion-motif-blockchain-explorer/build/mtfblocks    --port 8081 --testnet-url "http://139.162.32.245:8082" --enable-pusher --enable-emission-monitor'

# for testnet explorer
alias mtfblockstestnet='~/onion-motif-blockchain-explorer/build/mtfblocks -t --port 8082 --mainnet-url "http://139.162.32.245:8081" --enable-pusher --enable-emission-monitor'
```

These are aliases similar to those used for http://139.162.32.245:8081/ and http://139.162.32.245:8082/, respectively.

## Enable Motif emission

Obtaining current Motif emission amount is not straight forward. Thus, by default it is
disabled. To enable it use `--enable-emission-monitor` flag, e.g.,


```bash
mtfblocks --enable-emission-monitor
```
...
Every 10000 blocks, the thread will save current emission in a file, by default,
 in `~/.bitmotif/lmdb/emission_amount.txt`. For testnet or stagenet networks,
 it is `~/.bitmotif/testnet/lmdb/emission_amount.txt` or `~/.bitmotif/stagenet/lmdb/emission_amount.txt`. This file is used so that we don't
 need to rescan entire blockchain whenever the explorer is restarted. When the
 explorer restarts, the thread will first check if `~/.bitmotif/lmdb/emission_amount.txt`
...
```
Motif emission (fees) is 14485540.430 (52545.373) as of 1313448 block
```

The values given, can be checked using Motif daemon's  `print_coinbase_tx_sum` command.
...
Having the `crt` and `key` files, run `mtfblocks` in the following way:

```bash
./mtfblocks --ssl-crt-file=/tmp/server.crt --ssl-key-file=/tmp/server.key
```
...
```bash
# we use here official Motif project's donation address as an example
...
We use recipient's address (i.e. not our address from which we sent mtf to recipient).
...
```json
{
  "data": {
    ...
    "git_branch_name": "update_to_current_motif",
    "last_git_commit_date": "2017-07-25",
    "last_git_commit_hash": "a549f25",
    "motif_version_full": "0.10.3.1-ab594cfe"
  },
  "status": "success"
}
```
...
Return raw json block data, as represented in Motif.
...
Return raw json tx data, as represented in Motif.
...
Other examples can be found on  [github](https://github.com/motifexamples?tab=repositories).
