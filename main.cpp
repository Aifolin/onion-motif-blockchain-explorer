...
main(int ac, const char* av[])
{
    ...
    mtfeg::CmdLineOptions opts {ac, av};
    ...
    if (!mtfeg::get_blockchain_path(bc_path_opt, blockchain_path, nettype))
    {
    ...
    mtfeg::MicroCore mcore;
    cryptonote::Blockchain* core_storage;

    // initialize mcore and core_storage
    if (!mtfeg::init_blockchain(blockchain_path.string(),
    ...
    if (enable_emission_monitor == true)
    {
        ...
        mtfeg::CurrentBlockchainStatus::blockchain_path
                = blockchain_path;
        mtfeg::CurrentBlockchainStatus::nettype
                = nettype;
        mtfeg::CurrentBlockchainStatus::deamon_url
                = deamon_url;
        mtfeg::CurrentBlockchainStatus::set_blockchain_variables(
                &mcore, core_storage);
        ...
        mtfeg::CurrentBlockchainStatus::start_monitor_blockchain_thread();
    }


    mtfeg::MempoolStatus::blockchain_path
            = blockchain_path;
    mtfeg::MempoolStatus::nettype
            = nettype;
    mtfeg::MempoolStatus::deamon_url
            = deamon_url;
    mtfeg::MempoolStatus::set_blockchain_variables(
            &mcore, core_storage);

    mtfeg::MempoolStatus::network_info initial_info;
    strcpy(initial_info.block_size_limit_str, "0.0");
    strcpy(initial_info.block_size_median_str, "0.0");
    mtfeg::MempoolStatus::current_network_info = initial_info;
    ...
    // by tx searching threads that are launched for each user independently,
    // when they log back or create new account.
    mtfeg::MempoolStatus::mempool_refresh_time = mempool_refresh_time;
    mtfeg::MempoolStatus::start_mempool_status_thread();

    // create instance of page class which
    // contains logic for the website
    mtfeg::page mtfblocks(&mcore,
                          core_storage,
                          deamon_url,
                          nettype,
                          enable_pusher,
    ...
    CROW_ROUTE(app, "/")
    ([&]() {
        return mymtf::htmlresponse(mtfblocks.index2());
    });

    CROW_ROUTE(app, "/page/<uint>")
    ([&](size_t page_no) {
        return mymtf::htmlresponse(mtfblocks.index2(page_no));
    });

    CROW_ROUTE(app, "/block/<uint>")
    ([&](size_t block_height) {
        return mymtf::htmlresponse(mtfblocks.show_block(block_height));
    });

    CROW_ROUTE(app, "/block/<string>")
    ([&](string block_hash) {
        return mymtf::htmlresponse(
                mtfblocks.show_block(remove_bad_chars(block_hash)));
    });

    CROW_ROUTE(app, "/tx/<string>")
    ([&](string tx_hash) {
        return mymtf::htmlresponse(
                mtfblocks.show_tx(remove_bad_chars(tx_hash)));
    });
    ...
    if (enable_as_hex)
    {
        ...
        ([&](string tx_hash) {
            return crow::response(
                    mtfblocks.show_tx_hex(remove_bad_chars(tx_hash)));
        });

        CROW_ROUTE(app, "/ringmembershex/<string>")
        ([&](string tx_hash) {
            return crow::response(
                    mtfblocks.show_ringmembers_hex(remove_bad_chars(tx_hash)));
        });

        CROW_ROUTE(app, "/blockhex/<uint>")
        ([&](size_t block_height) {
            return crow::response(
                    mtfblocks.show_block_hex(block_height, false));
        });

        CROW_ROUTE(app, "/blockhexcomplete/<uint>")
        ([&](size_t block_height) {
            return crow::response(
                    mtfblocks.show_block_hex(block_height, true));
        });

//        CROW_ROUTE(app, "/ringmemberstxhex/<string>")
//        ([&](string tx_hash) {
//            return crow::response(
//              mtfblocks.show_ringmemberstx_hex(remove_bad_chars(tx_hash)));
//        });

        CROW_ROUTE(app, "/ringmemberstxhex/<string>")
        ([&](string tx_hash) {
            return mymtf::jsonresponse {
                mtfblocks.show_ringmemberstx_jsonhex(
    ...
    ([&](const crow::request& req) -> mymtf::htmlresponse
     {

        map<std::string, std::string> post_body
                = mtfeg::parse_crow_post_data(req.body);

        if (post_body.count("mtf_address") == 0
            || post_body.count("viewkey") == 0
            || post_body.count("tx_hash") == 0)
        {
            return string("mtf address, viewkey or tx hash not provided");
        }

        string tx_hash     = remove_bad_chars(post_body["tx_hash"]);
        string mtf_address = remove_bad_chars(post_body["mtf_address"]);
        ...
        string response = mtfblocks.show_my_outputs(
                                         tx_hash, mtf_address,
                                         viewkey, raw_tx_data,
                                         domain);

        return mymtf::htmlresponse(std::move(response));
    });

    CROW_ROUTE(app, "/myoutputs/<string>/<string>/<string>")
    ([&](const crow::request& req, string tx_hash,
        string mtf_address, string viewkey)
     {

        string domain = get_domain(req);

        return mymtf::htmlresponse(mtfblocks.show_my_outputs(
                                         remove_bad_chars(tx_hash),
                                         remove_bad_chars(mtf_address),
        ...
        ([&](const crow::request& req) -> mymtf::htmlresponse 
         {
            ...
            if (post_body.count("mtfaddress") == 0
                || post_body.count("txprvkey") == 0
                || post_body.count("txhash") == 0)
            {
                return string("mtf address, tx private key or "
            ...
            string mtf_address = remove_bad_chars(post_body["mtfaddress"]);
            ...
            return mymtf::htmlresponse(mtfblocks.show_prove(tx_hash,
                                        mtf_address,
                                        tx_prv_key,
                                        raw_tx_data,
                                        domain));
    ...
    ([&](const crow::request& req, string tx_hash,
         string mtf_address, string tx_prv_key) 
     {

        string domain = get_domain(req);

        return mymtf::htmlresponse(mtfblocks.show_prove(
                                    remove_bad_chars(tx_hash),
                                    remove_bad_chars(mtf_address),
    ...
    if (enable_pusher)
    {
        ...
        ([&]() {
            return mymtf::htmlresponse(mtfblocks.show_rawtx());
        });
        ...
        ([&](const crow::request& req) -> mymtf::htmlresponse
         {
            ...
            if (action == "check")
                return mymtf::htmlresponse(
                        mtfblocks.show_checkrawtx(raw_tx_data, action));
            else if (action == "push")
                return mymtf::htmlresponse(
                        mtfblocks.show_pushrawtx(raw_tx_data, action));
    ...
    if (enable_key_image_checker)
    {
        ...
        ([&]() {
            return mymtf::htmlresponse(mtfblocks.show_rawkeyimgs());
        });
        ...
        ([&](const crow::request& req) -> mymtf::htmlresponse
         {
            ...
            return mymtf::htmlresponse(
                    mtfblocks.show_checkrawkeyimgs(raw_data, viewkey));
        });
    }
    ...
    if (enable_output_key_checker)
    {
        ...
        ([&]() {
            return mymtf::htmlresponse(mtfblocks.show_rawoutputkeys());
        });
        ...
        ([&](const crow::request& req) -> mymtf::htmlresponse
         {
            ...
            return mymtf::htmlresponse(
                    mtfblocks.show_checkcheckrawoutput(raw_data, viewkey));
        });
    }
    ...
    ([&]() {
        return mymtf::htmlresponse(mtfblocks.mempool(true));
    });
...
//    });
    ...
    if (enable_js)
    {
        ...
        ([&]() {
            return mtfblocks.get_js_file("jquery.min.js");
        });

        CROW_ROUTE(app, "/js/crc32.js")
        ([&]() {
            return mtfblocks.get_js_file("crc32.js");
        });

        CROW_ROUTE(app, "/js/biginteger.js")
        ([&]() {
            return mtfblocks.get_js_file("biginteger.js");
        });

        CROW_ROUTE(app, "/js/crypto.js")
        ([&]() {
            return mtfblocks.get_js_file("crypto.js");
        });

        CROW_ROUTE(app, "/js/config.js")
        ([&]() {
            return mtfblocks.get_js_file("config.js");
        });

        CROW_ROUTE(app, "/js/nacl-fast-cn.js")
        ([&]() {
            return mtfblocks.get_js_file("nacl-fast-cn.js");
        });

        CROW_ROUTE(app, "/js/base58.js")
        ([&]() {
            return mtfblocks.get_js_file("base58.js");
        });

        CROW_ROUTE(app, "/js/cn_util.js")
        ([&]() {
            return mtfblocks.get_js_file("cn_util.js");
        });

        CROW_ROUTE(app, "/js/sha3.js")
        ([&]() {
            return mtfblocks.get_js_file("sha3.js");
        });
        ...
        ([&]() {
            ...
            return mtfblocks.get_js_file("all_in_one.js");
        });
    ...
    if (enable_json_api)
    {
        ...
        ([&](string tx_hash) {
            ...
            mymtf::jsonresponse r{mtfblocks.json_transaction(remove_bad_chars(tx_hash))};
        ...
        ([&](string tx_hash) {
            ...
            mymtf::jsonresponse r{mtfblocks.json_rawtransaction(remove_bad_chars(tx_hash))};
        ...
        ([&](string tx_hash) {
            ...
            mymtf::jsonresponse r{mtfblocks.json_detailedtransaction(remove_bad_chars(tx_hash))};
        ...
        ([&](string block_no_or_hash) {
            ...
            mymtf::jsonresponse r{mtfblocks.json_block(remove_bad_chars(block_no_or_hash))};
        ...
        ([&](string block_no_or_hash) {
            ...
            mymtf::jsonresponse r{mtfblocks.json_rawblock(remove_bad_chars(block_no_or_hash))};
        ...
        ([&](const crow::request &req) {
            ...
            mymtf::jsonresponse r{mtfblocks.json_transactions(
        ...
        ([&](const crow::request &req) {
            ...
            mymtf::jsonresponse r{mtfblocks.json_mempool(
                    remove_bad_chars(page), remove_bad_chars(limit))};
        ...
        ([&](string search_value) {
            ...
            mymtf::jsonresponse r{mtfblocks.json_search(remove_bad_chars(search_value))};
        ...
        ([&]() {
            ...
            mymtf::jsonresponse r{mtfblocks.json_networkinfo()};
        ...
        ([&]() {
            ...
            mymtf::jsonresponse r{mtfblocks.json_emission()};
        ...
        ([&](const crow::request &req) {
            ...
            mymtf::jsonresponse r{mtfblocks.json_outputs(
                    remove_bad_chars(tx_hash),
        ...
        ([&](const crow::request &req) {
            ...
            mymtf::jsonresponse r{mtfblocks.json_outputsblocks(
        ...
        ([&]() {
            ...
            mymtf::jsonresponse r{mtfblocks.json_version()};
    ...
    if (enable_autorefresh_option)
    {
        ...
        ([&]() {
            ...
            return mtfblocks.index2(page_no, refresh_page);
        });
    }
    ...
    if (enable_emission_monitor == true)
    {
        ...
        mtfeg::CurrentBlockchainStatus::m_thread.interrupt();
        mtfeg::CurrentBlockchainStatus::m_thread.join();
    ...
    mtfeg::MempoolStatus::m_thread.interrupt();
    mtfeg::MempoolStatus::m_thread.join();
